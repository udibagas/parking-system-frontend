<template>
  <div v-loading="loading">
    <el-form inline style="text-align: right">
      <el-form-item>
        <el-date-picker size="small" @change="requestData" v-model="date" format="dd/MMM/yyyy" value-format="yyyy-MM-dd"
          type="date">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
        <el-button size="small" type="primary" icon="el-icon-printer" @click="showPrintDialog = true">PRINT
          LAPORAN</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table show-summary stripe :data="report" :summary-method="getSummaries">
      ...
      <el-table-column prop="member.nomor_kartu" label="Nomor Kartu" header-align="center"
        align="center"></el-table-column>
      <el-table-column label="Plat Nomor" header-align="center" align="center">
        <template slot-scope="{row}">
          {{ row.member.vehicles.map((v) => v.plat_nomor).join(', ') }}
        </template>
      </el-table-column>
      <el-table-column label="Jumlah" header-align="right" align="right">
        <template slot-scope="{row}">
          Rp. {{ $decimal(row.jumlah) }}
        </template>
      </el-table-column>
    </el-table>

    <PrintDialog :show="showPrintDialog" @print="(printer_id) => printReport(printer_id)"
