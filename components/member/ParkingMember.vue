<template>
  <div>
    <el-form inline class="text-right" @submit.native.prevent>
      <el-form-item>
        <el-button size="small" @click="openForm({ vehicles: [], register_date: now, tarif: 0 })" type="primary"
          icon="el-icon-plus">TAMBAH MEMBER</el-button>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-button-group>
          <el-button icon="el-icon-download" size="small" type="primary" @click="exportData('member-parkir')"></el-button>
          <el-button icon="el-icon-printer" size="small" type="primary" @click="print"></el-button>
        </el-button-group>
      </el-form-item>
    </el-form>

    <el-table :data="tableData.data" stripe @row-dblclick="(row, column, event) => {
      selectedData = row
      showDetail = true
    }
      " height="calc(100vh - 310px)" @filter-change="filterChange" v-loading="loading" @sort-change="sortChange">
      <el-table-column type="index" :index="tableData.from" label="#"></el-table-column>
      <el-table-column :filters="[
        { value: 1, text: 'Aktif' },
        { value: 0, text: 'Nonaktif' },
      ]" column-key="status" prop="status" label="Status" sortable="custom" min-width="120px" header-align="center"
        align="center">
        <template slot-scope="scope">
          <el-tag size="small" effect="dark" style="width: 100%"
            :type="scope.row.status ? 'success' : 'info'">{{ scope.row.status ? 'Aktif' : 'Nonaktif' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="nama" label="Nama" sortable="custom" show-overflow-tooltip
        min-width="150px"></el-table-column>

      <el-table-column :filters="[
        { value: 'y', text: 'BERBAYAR' },
        { value: 'n', text: 'GRATIS' },
      ]" :filter-multiple="false" column-key="berbayar" prop="berbayar" label="Jenis" sortable="custom"
        min-width="100px" align="center" header-align="center">
        <template slot-scope="scope">
          {{ scope.row.berbayar ? 'BERBAYAR' : 'GRATIS' }}
        </template>
      </el-table-column>

      <el-table-column :filters="this.groupMemberList.map((g) => ({ value: g.id, text: g.nama }))
        " column-key="group_member_id" prop="group.nama" label="Group" sortable="custom" show-overflow-tooltip
        min-width="120px">
      </el-table-column>

      <el-table-column prop="nomor_kartu" label="Nomor Kartu" sortable="custom" show-overflow-tooltip
        min-width="150px"></el-table-column>
      ...
      <el-table-column prop="register_date" label="Tgl Daftar" sortable="custom" min-width="120px" align="center"
        header-align="center">
        <template slot-scope="scope">
          {{ $moment(scope.row.register_date).format('DD-MMM-YYYY') }}
        </template>
      </el-table-column>
      <el-table-column prop="expiry_date" label="Tgl Kedaluarsa" sortable="custom" min-width="150px" align="center"
        header-align="center">
        <template slot-scope="scope">
          {{ $moment(scope.row.expiry_date).format('DD-MMM-YYYY') }}
        </template>
      </el-table-column>
      <el-table-column prop="tarif" label="Tarif" sortable="custom" min-width="100px" header-align="right" align="right">
        <template slot-scope="scope">
          Rp. {{ $decimal(scope.row.tarif) }}
        </template>
      </el-table-column>
      <el-table-column prop="siklus_pembayaran" label="Siklus Bayar" sortable="custom" min-width="130px">
        <template slot-scope="scope">
      ...
      <el-table-column prop="phone" label="Nomor HP" sortable="custom" show-overflow-tooltip
      ...
      <el-table-column :filters="[
            { value: 'y', text: 'Ya' },
            { value: 'n', text: 'Tidak' },
          ]" :filter-multiple="false" column-key="expired" fixed="right" prop="expired" label="Expired"
        sortable="custom" min-width="120px" header-align="center" align="center">
        <template slot-scope="scope">
          <el-tag size="small" effect="dark" style="width: 100%"
            :type="scope.row.expired ? 'danger' : 'success'">{{ scope.row.expired ? 'Ya' : 'Tidak' }}</el-tag>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-zoom-in" @click.native.prevent="() => {
              ...
              <el-dropdown-item v-if="$auth.user.role == 1" icon="el-icon-delete"
    ...
    <el-dialog fullscreen center :visible.sync="showForm" :title="!!formModel.id ? 'EDIT MEMBER' : 'TAMBAH MEMBER'"
        ...
        <el-row :gutter="30">
          <el-col :span="8">
            <el-form-item label="Group" :class="formErrors.group_member_id ? 'is-error' : ''">
              <el-select v-model="formModel.group_member_id" placeholder="Group" style="width: 100%">
                <el-option v-for="t in groupMemberList" :value="t.id" :label="t.nama" :key="t.id"></el-option>
              </el-select>
            ...
            <el-form-item label="Jenis Kartu" :class="formErrors.card_type ? 'is-error' : ''">
              <el-select v-model="formModel.card_type" placeholder="Jenis Kartu" style="width: 100%">
                <el-option v-for="(t, i) in ['RFID', 'UHF']" :value="t" :label="t" :key="i"></el-option>
              </el-select>
          ...
          <el-col :span="8">
            <el-form-item label="Tanggal Daftar" :class="formErrors.register_date ? 'is-error' : ''">
              <el-date-picker format="dd-MMM-yyyy" value-format="yyyy-MM-dd" placeholder="Tanggal Daftar"
                v-model="formModel.register_date" style="width: 100%"></el-date-picker>
              <div class="el-form-item__error" v-if="formErrors.register_date">
                {{ formErrors.register_date[0] }}
              </div>
            </el-form-item>

            <el-form-item label="Jenis Anggota" :class="formErrors.berbayar ? 'is-error' : ''">
              <el-select v-model="formModel.berbayar" placeholder="Jenis Anggota" style="width: 100%">
                <el-option v-for="(t, i) in ['Gratis', 'Berbayar']" :value="Boolean(i)" :label="t" :key="i"></el-option>
              </el-select>
            ...
            <el-form-item label="Tarif (Rp)" :class="formErrors.tarif ? 'is-error' : ''">
              <el-input :disabled="!formModel.berbayar" type="number" placeholder="Tarif (Rp)"
                v-model="formModel.tarif"></el-input>
          ...
          <el-col :span="8">
            <el-form-item label="Siklus Pembayaran" :class="formErrors.siklus_pembayaran ? 'is-error' : ''">
              <el-input type="number" v-model="formModel.siklus_pembayaran" style="width: 30%"></el-input>
              <el-select v-model="formModel.siklus_pembayaran_unit" style="width: 66%; float: right; clear: right">
                <el-option v-for="(s, i) in siklus" :value="s.value" :label="s.label" :key="i"></el-option>
              </el-select>
            ...
            <el-form-item label="Tanggal Kedaluarsa" :class="formErrors.expiry_date ? 'is-error' : ''">
              <el-date-picker disabled format="dd-MMM-yyyy" value-format="yyyy-MM-dd" placeholder="Tanggal Kedaluarsa"
                v-model="expiry_date" style="width: 100%"></el-date-picker>
            ...
            <el-form-item label="Status">
              <el-select v-model="formModel.status" placeholder="Status" style="width: 100%">
                <el-option v-for="(t, i) in ['Nonaktif', 'Aktif']" :value="Boolean(i)" :label="t" :key="i"></el-option>
              </el-select>
      ...
      <el-table :data="formModel.vehicles">
        <el-table-column label="Jenis Kendaraan">
          <template slot-scope="scope">
            <el-select size="small" v-model="scope.row.jenis_kendaraan" placeholder="Jenis Kendaraan" style="width: 100%">
              <el-option v-for="kendaraan in jenisKendaraanList" :value="kendaraan.nama" :label="kendaraan.nama"
                :key="kendaraan.id"></el-option>
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="Plat Nomor">
          <template slot-scope="scope">
            <el-input v-model="scope.row.plat_nomor" placeholder="Plat Nomor" size="small"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Tipe">
          <template slot-scope="scope">
            <el-input v-model="scope.row.tipe" placeholder="Tipe" size="small"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column width="70px" align="right" header-align="right">
          <template slot="header">
            <el-button :disabled="setting.jml_kendaraan_per_kartu > 0 &&
              formModel.vehicles.length >= setting.jml_kendaraan_per_kartu
              " icon="el-icon-plus" @click="addVehicle" size="small" type="primary"></el-button>
          </template>
          <template slot-scope="scope">
            <el-button @click="deleteVehicle(scope.$index, scope.row.id)" icon="el-icon-delete" size="small" type="danger"
              plain></el-button>
          </template>
        </el-table-column>
      </el-table>
